...
def plot_hist(mapping, gt_dir, metric_dir):
    ...
    for key in mapping.keys():
        ...
        if query_category not in data.keys():
            data[query_category] = 0
        data[query_category] += 1

    # Separate keys and values
    items = list(data.keys())
    frequencies = list(data.values())

    # Assign a unique color for each item
    ...
    for idx, (item, freq) in enumerate(zip(items, frequencies)):
        plt.bar(idx, freq, color=color_mapping[item])

    # Remove x-axis ticks and labels
    plt.xticks([])

    # Create legend manually (inside the plot)
    handles = [plt.Rectangle((0, 0), 1, 1, color=color_mapping[item]) for item in items]
    plt.legend(handles, items, title='Query categories', loc='upper right', bbox_to_anchor=(0.95, 0.95))

    plt.xlabel('Query category')
    plt.ylabel('Frequency')
    plt.title('Frequency of Query categories')
    plt.tight_layout()
...
def run(args):
    ...
    F1_scores = {category : [] for category in splits.keys()}

    for category in splits.keys() :
        for seed in range(100):
            np.random.seed(seed)
            for vq in splits[category]['category_keys']:
    ...
    print(f'FS {fragment_size} | SP {summary_portion}')
    for category in F1_scores.keys():
        mean_f1_score = np.mean(F1_scores[category])
        print(f'{category}')
        print(mean_f1_score)
    return
...
"""
python /root/vidSum/src/TQVS_utils/PoR_evaluation_category.py \
--work_dir /root/TGVS \
--gt_dir /root/data/TGVS/GT \
--mapping_file /root/data/TGVS/TGVS_mapping.json \
--splits_file /root/data/TGVS/TGVS_Category_mapping.json \
--fragment_size 3 \
--summary_portion 36
"""