...
def run(args):
    ...
    for k in mapping.keys():
        if mapping[k]['video_id'] == my_model.video_name:
            random_key = k
            break
    
    assert random_key is not None, 'Erorr in mapping'
    
    with open(f'{og_PredMetaData_dir}/{random_key}.json', 'r') as json_file:
        data_temp = json.load(json_file)
        
    my_model.video_path = data_temp['video_path']
    my_model.video_fps = data_temp['video_fps']
    scene_scores = data_temp['scene_scores']
    start_frames = data_temp['scene_frames']
    my_model.n_frames = data_temp['n_frames']
    my_model.selected_sst = data_temp['sst']
    frames_consistency = data_temp['consistency']
    frames_dissimilarity = data_temp['dissimilarity']
